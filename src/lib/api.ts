...
function transformBlogPost(raw: any): BlogPost {
  return {
    ...
    excerpt: raw.excerpt ?? '',
    content: raw.content ?? '',
    image: raw.image_url ?? raw.image ?? '/images/placeholder-blog.jpg',
    author: {
      name: raw.author?.name ?? '',
      avatar: raw.author?.avatar_url ?? raw.author?.avatar ?? '/images/placeholder-avatar.jpg',
...
function transformCause(raw: any): Cause {
  return {
    ...
    description: raw.description,
    content: raw.content ?? '',
    image: raw.image_url ?? raw.image ?? '/images/placeholder-cause.jpg',
    gallery: raw.gallery ?? [],
...
function transformCentre(raw: any): Centre {
  return {
    ...
    address: raw.address ?? '',
    country: raw.country ?? '',
    image: raw.image_url ?? raw.image ?? '/images/placeholder-centre.jpg',
    description: raw.description ?? '',
    memberCount: raw.member_count ?? 0,
    leader: {
      name: raw.leader?.name ?? '',
      avatar: raw.leader?.avatar ?? '/images/placeholder-avatar.jpg',
...
function transformGalleryItem(raw: any): GalleryItem {
  return {
    ...
    url: raw.image_url ?? raw.image ?? '/images/placeholder-gallery.jpg',
    thumbnail: raw.thumbnail_url ?? raw.thumbnail ?? '/images/placeholder-gallery-thumb.jpg',
    type: raw.type,
...
function transformTestimonial(raw: any): Testimonial {
  return {
    ...
    name: raw.name,
    role: raw.role,
    avatar: raw.avatar_url ?? raw.avatar ?? '/images/placeholder-avatar.jpg',
    quote: raw.quote,
  };
}
...
function transformTeamMember(raw: any): TeamMember {
  return {
    ...
    role: raw.role,
    bio: raw.bio,
    image: raw.image_url ?? raw.image ?? '/images/placeholder-team.jpg',
  };
}
